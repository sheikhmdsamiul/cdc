...
const bnDivisions = Object.keys(bn_data);
const enDivisions = Object.keys(en_data);

for (let i = 0; i < bnDivisions.length; i++) {
  const bnDivName = bnDivisions[i];
  const enDivName = enDivisions[i];
  
  const bnDistricts = bn_data[bnDivName];
  const enDistricts = en_data[enDivName];
  
  const districts = [];
  const bnDistKeys = Object.keys(bnDistricts);
  const enDistKeys = Object.keys(enDistricts);
  
  for (let j = 0; j < bnDistKeys.length; j++) {
    const bnDistName = bnDistKeys[j];
    const enDistName = enDistKeys[j];
    
    const bnUpazilas = bnDistricts[bnDistName];
    const enUpazilas = enDistricts[enDistName];
    
    const upazilas = [];
    for (let k = 0; k < bnUpazilas.length; k++) {
      upazilas.push({
        bn: bnUpazilas[k],
        en: enUpazilas[k]
      });
