...
const QUERY_KEY = ["case-types"];

function CaseTypeForm({ initial, onSave, onClose, isBn }: {
  initial?: Partial<CaseType>;
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSave(form); }} className="space-y-4">
      <div className="space-y-2">
        ...
        <Input value={form.nameBn} onChange={(e) => setForm({ ...form, nameBn: e.target.value })} required placeholder="যেমন: হত্যা মামলা" />
      </div>
      <div className="space-y-2">
        <Label>{isBn ? "নাম (ইংরেজি)" : "Name (English)"}</Label>
        <Input value={form.nameEn} onChange={(e) => setForm({ ...form, nameEn: e.target.value })} required placeholder="e.g. Murder Case" />
      </div>
...
export default function CaseTypesPage() {
  ...
  const [deleting, setDeleting] = useState<CaseType | null>(null);

  const canView   = usePermission("case_types", "view");
  const canCreate = usePermission("case_types", "create");
  const canEdit   = usePermission("case_types", "edit");
  const canDelete = usePermission("case_types", "delete");

  const canManage = canCreate || canEdit || canDelete;

  const { data } = useQuery({
    queryKey: QUERY_KEY,
    queryFn: () => fetch("/api/case-types", { credentials: "include" }).then((r) => r.json()),
  });
  ...
  const createMutation = useMutation({
    mutationFn: async (d: any) => {
      const r = await fetch("/api/case-types", { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify(d) });
      if (!r.ok) throw new Error(await r.text());
      return r.json();
    },
    onSuccess: () => { qc.invalidateQueries({ queryKey: QUERY_KEY }); setOpen(false); toast({ title: isBn ? "মামলার ধরন যোগ করা হয়েছে" : "Case type added" }); },
    onError: (err) => toast({ title: isBn ? "ত্রুটি" : "Error", description: err.message, variant: "destructive" }),
  });

  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: number; data: any }) => {
      const r = await fetch(`/api/case-types/${id}`, { method: "PUT", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify(data) });
      if (!r.ok) throw new Error(await r.text());
      return r.json();
    },
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: number) => {
      const r = await fetch(`/api/case-types/${id}`, { method: "DELETE", credentials: "include" });
      if (!r.ok) throw new Error(await r.text());
      return r.json();
    },
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">{isBn ? "মামলার ধরন" : "Case Types"}</h1>
          <p className="text-muted-foreground text-sm mt-1">{isBn ? "মামলার ধরন পরিচালনা করুন" : "Manage case types for the ERP"}</p>
        </div>
        {canManage && (
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
              <Button><Plus className="h-4 w-4 mr-2" />{isBn ? "মামলার ধরন যোগ করুন" : "Add Case Type"}</Button>
            </DialogTrigger>
            <DialogContent className="max-w-md">
              <DialogHeader><DialogTitle>{isBn ? "নতুন মামলার ধরন" : "New Case Type"}</DialogTitle></DialogHeader>
              <CaseTypeForm isBn={isBn} onSave={(d) => createMutation.mutate(d)} onClose={() => setOpen(false)} />
            </DialogContent>
          </Dialog>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        {caseTypes.length === 0 && (
          <div className="col-span-full py-16 text-center text-muted-foreground text-sm">
            {isBn ? "কোনো মামলার ধরন পাওয়া যায়নি।" : "No case types found."}
          </div>
        )}
      </div>
      ...
      {editing && (
        <Dialog open={!!editing} onOpenChange={() => setEditing(null)}>
          <DialogContent className="max-w-md">
            <DialogHeader><DialogTitle>{isBn ? "মামলার ধরন সম্পাদনা" : "Edit Case Type"}</DialogTitle></DialogHeader>
            <CaseTypeForm isBn={isBn} initial={editing} onSave={(d) => updateMutation.mutate({ id: editing.id, data: d })} onClose={() => setEditing(null)} />
          </DialogContent>
        </Dialog>
      )}
